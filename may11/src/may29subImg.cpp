...
void bsLoc(const sensor_msgs::ImageConstPtr& msg, const sensor_msgs::ImageConstPtr& dimage,
    ...
    if (contours.size() > 0) {
        ...
        if (pred_pts.size() > ptNum) {
            ...
            marker.pose.position.x = pred_pts[ptNum] + transform.getOrigin().getX();
            marker.pose.position.y = pred_pts[ptNum+1] + transform.getOrigin().getY();
            marker.pose.position.z = pred_pts[ptNum+2] + transform.getOrigin().getZ();
...
int main (int argc, char **argv) {
    ...
    ros::Publisher vis_pub = nh.advertise<visualization_msgs::Marker>( "yupeng_pt", 0 );
    // // std::cout << "439\n" << std::endl;
    // visualization_msgs::Marker marker;
